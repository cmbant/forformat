...
| **I3** | Protected source is preserved byte-for-byte: string literal contents, doubled quotes, comment text (beyond the documented marker/commented-assignment transforms), CPP directive bodies, explicitly-defined macro spellings, non-UTF-8 bytes, and Hollerith-bearing statements. Preservation is of *content*, not single-token identity: a wrap may split one over-long literal into `//`-concatenated pieces at a whitespace boundary (see I5), but every content byte still survives, unreordered, across the split. |
