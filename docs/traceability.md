...
| `DeclarationCaseTests.test_implicit_identifiers_do_not_borrow_unrelated_project_case` | scope/project-case | `src/transform/passes/case_pass.rs` | `implicit_identifiers_do_not_borrow_unrelated_project_case` | covered by broader test — the implicit-none scope matrix lives in this test; the same file's `implicit_function_syntax_is_guarded_but_call_syntax_is_not` and `explicit_host_locals_and_use_names_still_canonicalize` cover the XFUN/XPROC call-guard and host-local/use-only halves: an implicit local borrowing an unrelated project spelling changes the emitted case |
| `DeclarationCaseTests.test_declaration_case_metadata_defaults_remain_compatible` | scope/project-case | — | — | excluded — private Python declaration-map defaults have no observable formatter or CLI equivalent; the release boundary is output bytes, syntax, and file workflow. |
...
| `SpacingTests.test_modernizes_multiline_array_constructors` | lexical | `tests/compatibility.rs` | `multiline_array_constructor_uses_valid_rust_delimiters` | covered by broader test — this test locks the `(/ /)` to `[ ]` bracket conversion across a continuation; `src/format/full.rs`'s `a_continued_format_statement_keeps_its_slash_before_the_paren` locks the companion case, a FORMAT statement's `/)` staying unconverted on a continuation line: either regressing turns brackets or a protected slash into the wrong bytes |
