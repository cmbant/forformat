...
///
/// Full and normalize-only modes serialize their final `Document` directly
/// into `out` rather than materializing a second complete output buffer.
...
/// they do avoid a separate final output buffer by writing the final document
/// directly into `out`.
